    ...
    try {
      ...
      for (const host of hosts) {
        try {
          ...
          totalGuests++;
          if (detail.online ?? host.online) onlineGuests++;
          if (detail.is_black || host.is_black) blockedGuests++;
          totalUpSpeed += detail.up_speed || host.up_speed || 0;
          totalDownSpeed += detail.down_speed || host.down_speed || 0;

          if (config.guestTimerEnabled && (detail.online ?? host.online)) {
            const seconds = detail.second || host.second || 0;
            ...
            if (now < expiresAt) timerActiveCount++;
          }
        } catch {
          continue;
        }
      ...
      console.error("Failed to get guest metrics:", error);
