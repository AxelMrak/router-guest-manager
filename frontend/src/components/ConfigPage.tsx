  ...
  if (isLoading) {
    return (
      <div className="bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-6 space-y-5">
        <div className="h-6 bg-white/[0.06] rounded animate-pulse w-32" />
        ...
        <div className="h-10 bg-white/[0.06] rounded animate-pulse w-24" />
      </div>
    );
  }
    ...
    <div className="bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            local.autoBlockEnabled ? "bg-blue-500" : "bg-white/[0.15]"
          }`}
        >
          <span
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-white/[0.06] border border-white/[0.10] rounded-lg px-3 py-2 text-white placeholder:text-white/20 focus:border-blue-400/50 focus:ring-0"
        />
      </div>
      ...
      {/* Guest Interfaces */}
      <div>
        <p className="text-sm text-white/70 mb-3">Guest Interfaces</p>
          ...
          {["ra1", "ra2", "rax1", "rax2"].map((iface) => (
            <label key={iface} className="flex items-center gap-2.5 cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded border-white/[0.15] bg-white/[0.06] text-blue-400 focus:ring-0 focus:ring-offset-0 accent-blue-400"
              />
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="bg-blue-500 hover:bg-blue-400 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg px-5 py-2 text-sm font-medium transition-colors"
        >
          {isSaving ? "Saving..." : "Save"}
        </button>
        <span
          className={`text-sm text-green-400 saved-fade ${saved ? "opacity-100" : "opacity-0"}`}
        >
          Saved!
        </span>
      </div>
